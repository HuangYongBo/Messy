...
/*                         notifier test      */
/*静态定义并初始化通知链头*/  
static BLOCKING_NOTIFIER_HEAD(test_notifier_list);  
  
/** 注册回调函数，加入一个回调函数节点 
 *  test_register_client - register a client notifier 
 *  @nb: notifier block to callback on events 
 */  
int test_register_client(struct notifier_block *nb)  
{  
    return blocking_notifier_chain_register(&test_notifier_list, nb);  
}  
EXPORT_SYMBOL(test_register_client);  
  
/** 注销回调函数，删除一个回调函数节点 
 *  test_unregister_client - unregister a client notifier 
 *  @nb: notifier block to callback on events 
 */  
int test_unregister_client(struct notifier_block *nb)  
{  
    return blocking_notifier_chain_unregister(&test_notifier_list, nb);  
}  
EXPORT_SYMBOL(test_unregister_client);  
  
/** 当源即framebuffer发生某种事件，调用该函数执行所有回调函数，通知其他子系统 
 * test_notifier_call_chain - notify clients of test_events 
 * 
 */  
int test_notifier_call_chain(unsigned long val, void *v)  
{  
    return blocking_notifier_call_chain(&test_notifier_list, val, v);  
}  
EXPORT_SYMBOL_GPL(test_notifier_call_chain);  
...
//#if GTP_CHANGE_X2Y
    swap(x, y);
//#endif
//    printk(KERN_ALERT"hhhhello\r\n");
    test_notifier_call_chain(0,NULL);
...
void gtp_reset_guitar(struct goodix_ts_data *ts, int ms)
{
    ...
    GPIO_MultiFun_Set(INT_GPIO, INT_FUNCTION); //This Pin has been set as output before,so we must set it as input.
        printk("[TP] Reset\n");
}
